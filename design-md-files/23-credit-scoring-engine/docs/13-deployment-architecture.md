...
The lifecycle above is the *mechanism*; this is the *process* that controls it. A model that decides who gets credit is a regulated risk artifact — governance failures here are regulatory findings, not engineering bugs. The framework follows model risk management practice (RBI expectations for credit models; conceptually aligned with SR 11-7).

### Roles (three lines of defense)

| Role | Owns | Cannot do |
|---|---|---|
| Model owner (Data Science) | Development, training data lineage, documentation, performance | Approve own model for production |
| Independent validator (Risk team) | Validation report review, fairness audit sign-off, `approval_token` issuance | Modify the model |
| Model risk committee | Model risk policy, inventory review, escalations, decommissioning approval | Day-to-day promotion decisions |

The `approval_token` in the promotion API is the technical enforcement of this separation: the registry rejects promotion without a token issued by the validator role (RBAC scope `model:approve`, never granted to data science).

### Model Validation Report — required per version, attached in the registry

1. Intended use and population (product types, score range, exclusions such as thin-file)
2. Training data lineage: sources, time window, exclusions, leakage checks
3. Performance: Gini/KS/AUC on out-of-time holdout, calibration by decile
4. **Fairness audit result** (launch gate per 00-requirements): disparate impact ratios across protected groups, with pass thresholds
5. Stability: PSI vs prior champion, sensitivity to top features
6. Limitations and compensating controls (e.g., thin-file routed to separate model)
7. Reason-code mapping review: codes remain accurate for the new feature importance ordering

### Inventory, Revalidation, Decommissioning

- **Model inventory** = the model registry, exported quarterly for the risk committee: every version with role history (SHADOW → CHALLENGER → CHAMPION → RETIRED), approval records, validation reports
- **Periodic revalidation**: annually per champion, or earlier when triggered by PSI > 0.2, approval-rate shift > 10%, or fairness monitoring breach — revalidation repeats steps 3–5 of the report
- **Material change definition**: new features, retraining on a new window, or threshold changes all create a new version requiring validation; config-only traffic-split changes do not
- **Decommissioning**: retired models stay loadable for 7 years — any historical score must be reproducible (model file + feature snapshot + version pinning already in the audit design)
- **Emergency rollback**: reverting to the previous validated champion needs no new validation (it retains its approval), only an incident record — this keeps the rollback path fast without bypassing governance

### What This Costs

One validator review per model version (days, not weeks, with the report template) and a quarterly committee review. Startups under-invest here until the first regulatory exam; the registry-as-inventory design keeps the marginal cost low because evidence is generated by the pipeline, not assembled manually.
