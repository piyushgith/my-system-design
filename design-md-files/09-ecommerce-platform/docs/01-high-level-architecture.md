...
Define the overall architectural style, service decomposition strategy, and communication patterns for the e-commerce platform. Justify the architecture choice and provide a clear migration path from startup to Hyperscaler scale.
...
## 11. Startup vs Hyperscaler Differences

| Aspect | Startup | Hyperscaler |
|---|---|---|
...
- **Why microservices over monolith at this scale?** At Amazon/Hyperscaler scale, the real driver is team independence. Each service can be deployed, scaled, and failed independently. The two-pizza team model only works if each team owns a deployable unit.
