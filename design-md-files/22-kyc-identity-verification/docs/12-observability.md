...
## Vendor Quality Drift Monitoring

Vendor OCR/liveness models change without notice — a silent vendor-side model update can shift accuracy, confidence distributions, and manual-review rates overnight. Treat vendor quality as a monitored time series, not a one-time integration test.

**Metrics (per vendor, per document type):**

- `kyc_ocr_field_mismatch_ratio` — OCR output vs operator-corrected value on manually reviewed cases (ground truth comes free from the manual review queue)
- `kyc_ocr_confidence_bucket` — confidence score distribution histogram; a shifting distribution with stable volume = vendor-side change
- `kyc_liveness_pass_rate` and confidence distribution — sudden pass-rate jump is as suspicious as a drop (could mean weakened spoof detection)
- `kyc_manual_review_rate` by routing reason — rising `low_ocr_confidence` share is the earliest drift signal
- `kyc_vendor_cost_total{vendor, check_type}` — cost per completed verification (see 00-requirements Vendor Cost Model)

**Detection:** weekly job compares each metric's 7-day window against the trailing 90-day baseline; > 2σ shift opens a vendor-quality ticket automatically. Golden-set probing: replay a fixed, consented set of ~200 synthetic/test documents through each vendor weekly — identical inputs isolate vendor-side change from traffic-mix change.

**Response:** drift confirmed → raise OCR confidence threshold (more manual review, no correctness loss), notify the vendor with the golden-set evidence, and if unresolved shift routing weight to the fallback vendor via the vendor abstraction layer.
