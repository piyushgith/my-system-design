...
**Hyperscaler Reality**: YouTube has a "lazy fan-out" model — notifications are not pushed to all subscribers immediately. High-subscriber creators use a "pull on demand" approach where the notification is written to a shared store and pulled when the subscriber opens the app.
