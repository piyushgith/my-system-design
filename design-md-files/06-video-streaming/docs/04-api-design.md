...
**Hyperscaler Detail**: YouTube and Google Drive use a similar resumable upload protocol. The client uploads chunks directly to S3 using presigned URLs — the Upload Service never proxies the bytes, avoiding bandwidth bottleneck on application servers.
