...
- **How does a startup approach this vs Hyperscaler?** Startup: Railway/Render, managed Postgres, Upstash Redis, no Kubernetes, deploy button. Hyperscaler: full EKS, Terraform, multi-region, change freezes, canary with automated rollback. The architecture is the same; the operational sophistication scales with team size
