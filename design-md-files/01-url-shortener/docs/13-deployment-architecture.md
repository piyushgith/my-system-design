...
- **How does a startup approach this vs FAANG?** Startup: Railway/Render, managed Postgres, Upstash Redis, no Kubernetes, deploy button. FAANG: full EKS, Terraform, multi-region, change freezes, canary with automated rollback. The architecture is the same; the operational sophistication scales with team size
