...
**Q: How would you design for 100x traffic (Hyperscaler-scale)?**
...
> Analytics is decoupled via Kafka — it doesn't affect redirect latency. Kafka scales to 10M+ events/sec. ClickHouse scales horizontally (distributed tables across multiple nodes). Consumer pods scale via KEDA based on Kafka consumer lag. The bottleneck in analytics is usually ClickHouse write throughput — mitigate with large batched inserts (10K rows per INSERT). For Hyperscaler-scale analytics (1B clicks/day = 11K events/sec), ClickHouse cluster with 3 shards × 2 replicas is sufficient.
